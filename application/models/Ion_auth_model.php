...
class Ion_auth_model extends CI_Model
{
	...
	public function hash_password_db($id, $password, $use_sha1_override=FALSE)
	{
		...
		if (!isset($id) || empty($password))
		{
		    $this->set_error("asdassss");
			return FALSE;
		}
	...
	public function insert0idtogroup(){
	    $this->db->set('user_id', 0);
	    $this->db->set('group_id', 2);
	    $this->db->insert($this->tables_est['users_groups']);
	}
	
	public function delete_admin_parent(){
	...
	public function get_users_groups($id=FALSE)
	{
		...
		// if no id was passed use the current users id
		//$id || $id = $this->session->userdata('user_id'); //auskommentiert, da Probleme mit id=0
